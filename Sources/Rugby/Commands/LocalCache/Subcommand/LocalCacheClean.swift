...
private func allFolders(_ location: String) throws -> [Folder] {
	...
	let firstLevelFolders = remoteFolder.subfolders
	let secondLevelFolders = firstLevelFolders.map(\.subfolders).flatMap { $0 }
	let thirdLevelFolders = secondLevelFolders.map(\.subfolders).flatMap { $0 }
	let nonBuildFolders = thirdLevelFolders.filter { $0.name != .buildFrameworkFolder }
	var allFolders = nonBuildFolders.map(\.subfolders).flatMap { $0 }

	assert(allFolders.filter({ $0.path.contains(String.buildFrameworkFolder) }).isEmpty)

	let buildFrameworkFolders = thirdLevelFolders.filter { $0.name == .buildFrameworkFolder }
	let frameworkFirstLevel = buildFrameworkFolders.map(\.subfolders).flatMap { $0 }
	let frameworkSecondLevel = frameworkFirstLevel.map(\.subfolders).flatMap { $0 }
	let allFrameworkFolders = frameworkSecondLevel
